...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.isButton() && interaction.customId.startsWith('ticket_open_')) {
      ...
      if (existing) return interaction.reply({ content: `❌ عندك تذكرة مفتوحة! ${existing}`, ephemeral: true });
      ...
      const embed = new EmbedBuilder()
        .setColor('#5865F2').setTitle('🎫 تذكرة جديدة')
    ...
    if (interaction.isButton() && interaction.customId.startsWith('ticket_claim_')) {
      ...
      if (!hasRole) return interaction.reply({ content: '❌ فقط فريق الدعم يقدر يستلم التذكرة!', ephemeral: true });
    ...
    if (interaction.isButton() && interaction.customId === 'ticket_rename') {
      ...
      const input = new TextInputBuilder().setCustomId('new_name').setLabel('الاسم الجديد')
        .setStyle(TextInputStyle.Short).setPlaceholder('ticket-مشكلتي').setMaxLength(50).setRequired(true);
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'ticket_rename_modal') {
      ...
      await interaction.reply({ embeds: [new EmbedBuilder().setColor('#00FF00').setDescription(`✅ تم تغيير اسم التذكرة لـ **ticket-${newName}**`)] });
      return;
    }
    ...
    if (interaction.isButton() && interaction.customId === 'ticket_close_btn') {
      try {
        ...
        await interaction.channel.send({ embeds: [new EmbedBuilder().setColor('#FF0000').setDescription('🔒 سيتم إغلاق هذه التذكرة خلال 5 ثواني...')] });
